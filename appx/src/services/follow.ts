...
export const getFollowedUsers = async (): Promise<{ ID: string; UserName: string }[]> => {
    try {
        // FirebaseからユーザーIDを取得
        const auth = getAuth();
        const currentUser = auth.currentUser;
        ...
        // GET リクエストを送信
        const response = await fetch(`${backendUrl}/follow/followed?followed_id=${userId}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.message || "Failed to fetch followedusers");
        }
